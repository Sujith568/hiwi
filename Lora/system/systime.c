...
void SysTimeSet( SysTime_t sysTime )
{
    ...
    if (localtime(&epoch_time ))
    {
		ts = localtime(&epoch_time);
        time_struct currentTime;
        currentTime.hour       = ts->tm_hour; // 0 to 23
        currentTime.minute     = ts->tm_min; // 0 to 59
        currentTime.second     = ts->tm_sec; // 0 to 59
        currentTime.hundredth = sysTime.SubSeconds / 10;
        currentTime.dayOfMonth = ts->tm_mday; // 1 to 31
        currentTime.month      = ts->tm_mon; // 0 to 11
        currentTime.year       = ts->tm_year + 1900 - 2000; // years since 2000

		
		am_devices_am1805_set_time(&currentTime);
		time_struct testTime;
		am_devices_am1805_get_time(&testTime);
		requireTimeRequest = false;
    }
...
SysTime_t SysTimeGet( void )
{
	...
	struct tm ts;
	ts.tm_hour = currentTime.hour;
	ts.tm_min = currentTime.minute;
	ts.tm_sec = currentTime.second;
	ts.tm_mday = currentTime.dayOfMonth;
	ts.tm_mon = currentTime.month;
	ts.tm_year = currentTime.year + 100;		//years since 1900
    //RtcBkupRead( &seconds, &subSeconds );
	time_t timeSinceEpoch = mktime(&ts);
	calendarTime.Seconds = timeSinceEpoch;
